...
export class AnswersService {
  ...
  constructor(private localService: LocalService) {
    if (this.localService.getData('artistsAnswers')) {
      this.artistsAnswers = JSON.parse(
        <string>this.localService.getData('artistsAnswers')
      );
    }
    if (this.localService.getData('picturesAnswers')) {
      this.picturesAnswers = JSON.parse(
        <string>this.localService.getData('picturesAnswers')
      );
    }
  }
  ...
  getCountRightAnswers(nameCategory: string, numCategory: number): number {
    let answers =
      nameCategory == 'artists'
        ? this.artistsAnswers[numCategory]
        : this.picturesAnswers[numCategory];
